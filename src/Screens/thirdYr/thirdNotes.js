...
export default class ThirdNotes extends Component {
    ...
    componentDidMount() {
        firestore().collection('Third_subject').get().then(querySnapShot => {
            let docs = [];
            querySnapShot
    ...
    Document_Upload = async () => {
        ...
        setTimeout(async () => {
            try {
                ...
                setTimeout(() => {
                    const task = storage().ref(`thirdSubjectDocs/${res.name}`).putFile(res.uri);
                    ...
                    task.then(async () => {
                        ...
                        const downURl = await storage().ref('thirdSubjectDocs').child(`${res.name}`).getDownloadURL();
                        firestore()
                            .collection('Third_subject')
                            .doc(res.name)
